...
\label{maindefinition}
    The \emph{Intuitive Ordinal Intelligence} of an AGI $X$ is the smallest computable
...
One of the most important properties of the ordinal numbers is
\emph{well-foundedness}: there is
...
notation was obtained using certain tricks, then \emph{any} AGI who used those
tricks to notate $\alpha$ should also be able to iterate those same tricks.
...
We will say more.
As an analogy, imagine a ladder which different AGIs
...
ladder if there were many different AGIs capable of scaling the whole
...
use of a \emph{fast-growing hierarchy}
\cite{hardy1904theorem} \cite{wainer1987provably}
in extend Hibbard's ladder into the transfinite and reduce (or eliminate?)
...
word ``ordinal''
in its everyday sense, i.e.\ as a synonym for ``natural number'').}.
A good ladder
...
a set of mathematical facts.}. Some AGI might be good at video games
...
without needing to run it. In the same way, if $X$ knows the sourcecode
...
ZFC, etc. G\"odel's incompleteness theorem shows that for any computably
enumerable set of axioms capable of basic arithmetic, there is always a
...
statement.
Thus, it is absurd to imagine there is some fixed computably enumerable
...
populations,
see \cite{strannegaard2018learning}, \cite{yoshida2017homeostatic},
\cite{beyret2019animal}.}, the Knight-Darwin
