...
Let $G$ be the directed graph whose nodes are all the
living organisms (past, present, and future), with an
edge directed from organism $u$ to organism $v$ if and only if
$u$ is a biological parent of $v$. Then $G$ does not contain an
infinite directed path of nodes each of which has $<2$ parents.
\end{principle}
