...
\title{AGI and the Knight-Darwin Law: why idealized AGIs need to collaborate
...
Under idealized assumptions, our answer is:
...
By proving that a certain universal intelligence measure decreases when an
...
We call an AGI \emph{truthful} if the things it knows (or could eventually
deduce\footnote{Identifying what is known with what could eventually
be deduced is of course only valid in the idealized context of this paper.
In Section 6 of \cite{alexander2019measuring} we suggest a hypothetical way in which
these idealized results could be applied to certain less-idealized machines.}) are true,
...
that $X$ knows the truthfulness\footnote{One reason the creator might want the created
AGI truthful is because if the created AGI eventually deduces a contradiction, such as
$1=0$, then, at least in classical mathematics where a contradiction proves everything,
the created AGI could immediately deduce things like ``I should kill my creator''.}
...
AGIs which presumably comprehend
...
thereof\footnote{For any particular ordinal notation, we see no reason
...
how AGIs create new AGIs, we can make progress toward
