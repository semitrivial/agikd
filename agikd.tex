...
and some appeared (weaker and less approachably) in the author's
dissertation \cite{alexanderdissert}.
...
To make Definition \ref{literalnotationdef} fully rigorous, one would need
...
The following theorem provides a relationship\footnote{Possibly formalizing a
...
It would be straightforward to extend Theorem \ref{maintheorem} to cases where $X$ creates
$Y$ non-deterministically. Suppose $X$ creates $Y$ using random numbers, such that $X$ knows
$Y$ is
...
If each $X_i$ creates $X_{i+1}$
so as to know $X_{i+1}$ is truthful,
then
...
questions about the relationship between the set of AGIs initially created by
...
Notations. We argued that this notion measures (a type of) intelligence.
