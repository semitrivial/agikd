...
\begin{proof}
    ...
    $c$ of an AGI, simulates that AGI waking up in an empty room and being commanded
    ...
    circumstances. Having written $Sim(c)$, $X$ could then obtain $P$ by plugging $Y$'s
    code into $Sim$. Note that nowhere in this process do we require $X$ to actually
...
are \emph{well-founded}\footnote{This is essentially true by definition,
hinging on the question of what ordinals formally are, which is beyond the scope of
this paper. But if the reader is willing to accept that mathematical induction works
on the ordinals like it does on the naturals, the following argument will convince
the reader there is no infinite descending sequence (IDS) of ordinals.
Certainly there is no infinite descending sequence (IDS) starting at $0$.
Let $\alpha>0$ be an ordinal and inductively assume there is no IDS
starting at $\beta$ for any $\beta<\alpha$. If there were an IDS
$\alpha>\alpha'>\alpha''>\cdots$ starting at $\alpha$, then
$\alpha'>\alpha''>\cdots$ would be an IDS starting at $\alpha'$,
...
would be an infinite strictly-descending sequence of ordinals, which would
